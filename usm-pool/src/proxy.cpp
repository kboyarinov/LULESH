...
static inline void* InternalOperatorNew(size_t sz) {
    InitOrigPointers();
    void *res = safer_aligned_malloc(sz, sizeof(void*), orig_malloc);
#if TBB_USE_EXCEPTIONS
    while (!res) {
        ...
        res = safer_aligned_malloc(sz, sizeof(void*), orig_malloc);
}
...
inline void InitOrigPointers()
{
    ...
    if (!origFuncSearched.load(std::memory_order_acquire)) {
        ...
        // sycl::_V1::queue::submit_impl(std::function<void (sycl::_V1::handler&)>, sycl::_V1::detail::code_location const&)
...
{
    InitOrigPointers();
    return safer_aligned_malloc(size, sizeof(void*), orig_malloc);
}

void *PREFIX(calloc)(ZONE_ARG size_t num, size_t size) __THROW
{
    char *res = static_cast<char*>(safer_aligned_malloc(num * size, sizeof(void*), orig_malloc));
...
{
    InitOrigPointers();
    safer_free(object, (void (*)(void*))orig_free, (sycl_free_type)orig_sycl_free);
}
...
{
    return safer_aligned_malloc(size, alignment, orig_malloc);
}
...
void *PREFIX(valloc)(ZONE_ARG size_t size) __THROW
{
    ...
    return safer_aligned_malloc(size, memoryPageSize, orig_malloc);
}
...
{
    *memptr = safer_aligned_malloc(size, alignment, orig_malloc);
    return 0;
}
...
{
    InitOrigPointers();
    safer_free(ptr, (void (*)(void*))orig_libc_free, (sycl_free_type)orig_sycl_free);
}
...
void operator delete(void* ptr) noexcept {
    InitOrigPointers();
    safer_free(ptr, (void (*)(void*))orig_free, (sycl_free_type)orig_sycl_free);
}
void operator delete[](void* ptr) noexcept {
    InitOrigPointers();
    safer_free(ptr, (void (*)(void*))orig_free, (sycl_free_type)orig_sycl_free);
...
void* operator new(std::size_t size, std::align_val_t al) {
    InitOrigPointers();
    return safer_aligned_malloc(size, size_t(al), orig_malloc);
}
void* operator new[](std::size_t size, std::align_val_t al) {
    InitOrigPointers();
    return safer_aligned_malloc(size, size_t(al), orig_malloc);
